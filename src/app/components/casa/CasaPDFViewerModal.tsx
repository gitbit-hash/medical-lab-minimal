...
interface CasaPDFViewerModalProps {
  isOpen: boolean;
  onClose: () => void;
  testId: string;
  patientPhone?: string; // Added phone prop for WhatsApp sharing
}

export function CasaPDFViewerModal({ isOpen, onClose, testId, patientPhone }: CasaPDFViewerModalProps) {
  const [pdfUrl, setPdfUrl] = useState<string | null>(null);
  const [pdfBlob, setPdfBlob] = useState<Blob | null>(null); // State to store blob for sharing
  ...
  useEffect(() => {
    async function loadPdf() {
      ...
      // Using CASA specific API endpoint
      const url = `/api/generate-casa-pdf?testId=${testId}`;
      ...
      try {
        ...
        setPdfBlob(blob); // Store blob for sharing
        setPdfUrl(blobUrl);
        setProgress(100);
      } catch (err) {
  ...
  const handleDownload = () => {
    ...
    link.download = `CASA-Report-${testId}.pdf`;
  ...
  // New function to handle WhatsApp sharing (Same as regular modal)
